...
2. Чужой или старый скрипт: `luatool.exe migrate "<файл>" --prefix <2-3 буквы>`. Утилита встраивает qLocalizer, разворачивает ключи с точками в плоские и меняет их по всему коду, включая собираемые через `..`. Она выносит текст меню в словарь, убирает ручной `Wrap` и ставит `WrapLibrary(Menu)`. Повторный запуск ничего не меняет. Логику скрипта не трогай. Разбери отчет:
   ...
   - `STOPPED`: файл не тронут, потому что в нем несколько словарей (несколько скриптов в одном файле) или словарь собирается кодом. Такой файл переводи вручную или сначала раздели.
...
4. Сигнатуры бери только из `references/api/`, не по памяти. У Umbrella свой API: он не совпадает ни с серверным Lua API Valve, ни с API других читов. Функции игры вызываются через модуль: `NPC.GetMana(npc)`, `Entity.GetAbsOrigin(unit)`, `Ability.CastPosition(ability, pos)`. Запись `npc:GetMana()` не работает. Через двоеточие вызываются только методы объектов меню и `Vector`, `Vec2`, `Angle`, `Color`. Имена способностей, талантов, предметов, героев и юнитов, `AbilityBehavior`, время каста и спецзначения из `AbilityValues` бери только из данных игры, никогда по памяти: `luatool.exe data ability|item|unit <имя>`, поиск `luatool.exe data find <часть имени>`, подробно в `references/game-data.md`.
5. После каждой правки: `luatool.exe check "<файл>"`. Синтаксис проверяется точно как в Lua 5.4, включая лимит в 200 локальных. Кроме того, проверка сверяется со справочником и ловит:
   ...
   - имена, которые нигде в файле не объявлены, и `local`, использованные раньше своего объявления;
   - имена способностей, предметов, героев и юнитов и ключи спецзначений, которых нет в данных игры;
   - обращения к `ai_bridge` без проверки на его наличие;
   - длинные тире.
   Все `WARN` исправь. `INFO` только для сведения. Если в уже выпущенном скрипте меняешь имена вкладок, групп или пунктов, до правки сохрани копию файла во временную папку и проверь `luatool.exe check "<файл>" --against "<копия>"`: у пропавших имен у игроков сбросятся настройки и бинды, предупреди об этом.
6. Тестирует пользователь в игре. Моки, симуляции и тестовые прогоны не пиши. Когда пользователь перезагрузил скрипты и что-то не работает, запусти `luatool.exe check "<файл>" --log`: команда покажет из `%cheat_dir%/debug.log` после последней перезагрузки ошибки этого скрипта, его собственные строки `[Имя] ...` и картинки, которые он не смог загрузить. Правки строй по его наблюдениям из игры.
7. Если утилита напечатала строку `UPDATE:`, в конце ответа один раз скажи пользователю, что вышла новая версия скилла, и дай ссылку из этой строки. Установленную версию и наличие новой показывает `luatool.exe version`.
8. Обновление ставь, только если пользователь попросил: скачай `https://github.com/But3rflys/umbrella-work/releases/download/umbrella-lua-v<версия>/umbrella-lua.zip`, распакуй во временную папку и замени папку `<skill>` целиком папкой `umbrella-lua` из архива. Потом проверь `luatool.exe version`.
...
- Отладочный лог пиши через `Log.Write("[Имя] ...")` только под переключателем отладки и только при смене состояния, не каждый кадр. `print` и `Log.Write` сами превращают таблицы в текст. Если поведение игры неизвестно, пиши в лог все состояние юнита: список модификаторов и результат каждой проверки. Так причина находится с одного теста.
- Имена и числа из игры (способности, предметы, герои, юниты, спецзначения, кулдауны, время каста) бери только из `luatool.exe data`. Если имени там нет, не угадывай, а спроси пользователя. Исключение: кастомные режимы вроде Dota 1x6 со своими именами `..._custom`.
...
| Данные игры: способности, предметы, герои, юниты | `references/game-data.md`, команда `luatool.exe data` |
